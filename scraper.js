...
const parser = new Parser({
  headers: {
    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) WetFloorWatch-Precision/20.0',
...
const FEEDS = [
  { url: "https://news.google.com/rss/search?q=Hamilton+Ontario+(police+OR+crime+OR+charges+OR+investigation)+when:6m&hl=en-CA&gl=CA&ceid=CA:en", type: "emergency", sourceName: "Official Police Dispatch" },
  { url: "https://news.google.com/rss/search?q=Hamilton+Ontario+(news+OR+shooting+OR+stabbing+OR+assault+OR+drug+OR+encampment+OR+fire)+when:3m&hl=en-CA&gl=CA&ceid=CA:en", type: "news", sourceName: "Local News Network" },
  { url: "https://news.google.com/rss/search?q=Hamilton+Spectator+(news+OR+incident+OR+police+OR+update)+when:3m&hl=en-CA&gl=CA&ceid=CA:en", type: "news", sourceName: "The Hamilton Spectator" },
  { url: "https://www.reddit.com/r/Hamilton/search.rss?q=needle+OR+drug+OR+tent+OR+encampment+OR+paraphernalia+OR+overdose+OR+police+OR+incident&restrict_sr=on&sort=new&t=year", type: "unverified", sourceName: "Reddit r/Hamilton" },
...
async function verifyAndExtract(item, feedType, sourceName) {
  ...
  // Fallback default coordinate in Downtown Hamilton so local news and articles always generate valid pins
  const pinData = matchedCorridor || { name: "Hamilton Central Core", lat: 43.2570, lng: -79.8700 };
...
async function run() {
  console.log("Running Deep Multi-Source Ingestion...");
  let count = 0;
  ...
  for (const feed of FEEDS) {
    try {
      ...
      for (const item of (parsedFeed.items || []).slice(0, 60)) {
        ...
        count++;
        console.log(`[Synced] ${intel.category} -> ${intel.source}`);
      }
  ...
  console.log(`Ingestion Complete. Synchronized ${count} intelligence pins.`);
}
