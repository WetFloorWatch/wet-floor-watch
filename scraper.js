...
async function run() {
  console.log("Starting quota-free Greater Hamilton safety intelligence generator...");
  ...
  for (const feedUrl of FEEDS) {
    try {
      ...
      for (const item of (feed.items || []).slice(0, 8)) {
        // Unique salt ensuring every run generates fresh documents
        ...
        if (text.includes('assault') || text.includes('fight') || text.includes('attack') || text.includes('crime') || text.includes('theft')) {
          category = 'assaults';
        } else if (text.includes('drug') || text.includes('needle') || text.includes('encampment') || text.includes('overdose')) {
          category = 'drugs';
        } else if (text.includes('shooting') || text.includes('gun') || text.includes('weapon')) {
          category = 'shootings';
        } else {
          category = 'emergency';
        }
        ...
        await docRef.set({
          category: category,
          source: `Verified News Feed (${zone.name})`,
