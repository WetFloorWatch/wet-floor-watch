...
const parser = new Parser({
  headers: {
    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) WetFloorWatch/12.0',
...
const FEEDS = [
  // Official Police & Emergency Feeds
  { url: "https://news.google.com/rss/search?q=site:hamiltonpolice.on.ca+OR+%22Hamilton+Police+Service%22+(drug+OR+weapons+OR+assault+OR+stabbing+OR+arrest)+when:2y&hl=en-CA&gl=CA&ceid=CA:en", type: "emergency", sourceName: "Official Police Dispatch" },
  
  // Local News Networks
  { url: "https://news.google.com/rss/search?q=Hamilton+Ontario+(shooting+OR+stabbing+OR+weapons+OR+homicide)+when:2y&hl=en-CA&gl=CA&ceid=CA:en", type: "news", sourceName: "Local News Network" },
  { url: "https://news.google.com/rss/search?q=Hamilton+Ontario+(drug+OR+needle+OR+encampment+OR+overdose)+when:2y&hl=en-CA&gl=CA&ceid=CA:en", type: "news", sourceName: "Local News Network" },
  { url: "https://news.google.com/rss/search?q=Hamilton+Ontario+(crash+OR+collision+OR+fire+OR+EMS+OR+hazard)+when:1y&hl=en-CA&gl=CA&ceid=CA:en", type: "verified", sourceName: "Verified Reports & Roadwork" },
  
  // Public Social Media (Reddit, FB, IG, X)
  { url: "https://www.reddit.com/r/Hamilton/search.rss?q=drug+OR+needle+OR+encampment+OR+tent+OR+overdose&restrict_sr=on&sort=new&t=year", type: "unverified", sourceName: "Reddit r/Hamilton" },
  { url: "https://www.reddit.com/r/Hamilton/search.rss?q=police+OR+assault+OR+stabbing+OR+weapons+OR+shooting&restrict_sr=on&sort=new&t=year", type: "unverified", sourceName: "Reddit r/Hamilton" },
  { url: "https://news.google.com/rss/search?q=Hamilton+(site:facebook.com+OR+site:instagram.com+OR+site:twitter.com)+(needle+OR+drug+OR+encampment+OR+assault+OR+police)+when:2y&hl=en-CA&gl=CA&ceid=CA:en", type: "unverified", sourceName: "Public Social Media Watch" }
];
...
const EXACT_STREET_WHITELIST = [
  ...
  { names: ['york blvd', 'york boulevard', 'bay st'], name: "York Blvd & Bay St", lat: 43.2625, lng: -79.8732 },
  ...
  { names: ['westdale', 'mcmaster', 'sterling'], name: "Westdale / McMaster Perimeter", lat: 43.2600, lng: -79.9100 },
  { names: ['queenston rd', 'queenston road'], name: "Queenston Rd Corridor", lat: 43.2250, lng: -79.7650 },
  ...
  { names: ['dundas', 'king st w dundas'], name: "Dundas Core", lat: 43.2660, lng: -79.9550 }
];
  ...
  try {
    const chatCompletion = await groq.chat.completions.create({
      messages: [
        ...
        { role: "user", content: `Is this a public safety hazard, crime, drug use, needle, encampment, fire, or police report in Hamilton? Text: "${leadText}"` }
      ],
  ...
  if (geminiCallCount < MAX_GEMINI_CALLS) {
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: `Is this a safety hazard, drug incident, encampment, or emergency in Hamilton? Reply JSON: {"valid": true}. Text: "${leadText}"`
      });
...
async function verifyAndExtract(item, feedType, sourceName) {
  ...
  return {
    category: feedType, // PRESERVES FEED TYPE: "emergency", "news", "verified", or "unverified"
    lat: matchedCorridor.lat + (Math.random() - 0.5) * 0.003,
    lng: matchedCorridor.lng + (Math.random() - 0.5) * 0.003,
...
async function run() {
  console.log("Running Multi-Source Categorized Ingestion...");
  let count = 0;
  ...
  for (const feed of FEEDS) {
    try {
      ...
      for (const item of (parsedFeed.items || []).slice(0, 50)) {
        ...
        count++;
        console.log(`[Mapped Pin] ${intel.category} -> ${intel.source}`);
      }

      // 2.5 second sleep prevents Reddit HTTP 429 rate limit blocks
      await sleep(2500);
    } catch (e) {
      console.error(`Feed Error (${feed.sourceName}):`, e.message);
    }
  }
  console.log(`Ingestion Complete. Deployed ${count} categorized pins. Gemini calls used: ${geminiCallCount}/${MAX_GEMINI_CALLS}`);
}
